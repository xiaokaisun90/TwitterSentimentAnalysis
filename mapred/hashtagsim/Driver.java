...
public class Driver {
	...
	public static void main(String args[]) throws Exception {
		...
		//		output);
		getHashtagAllFeatureVector(input, tmpdir + "/feature_vector_all");

        /*
         * Sort
		getHashtagAllSimilarities(tmpdir + "/feature_vector_all", tmpdir + "/unsorted_similarity");
        sortHashTagBySimilarity(tmpdir + "/unsorted_similarity", output);
        */
		getHashtagAllSimilarities(tmpdir + "/feature_vector_all", output + "_unsorted");
	}
	 ...
	 * @throws Exception
	 */
	private static void getHashtagAllFeatureVector(String input, String output)
			throws Exception {
		Optimizedjob job = new Optimizedjob(new Configuration(), input, output,
				"Get feature vector for all words");
		job.setClasses(HashtagAllMapper.class, HashtagAllReducer.class, null);
		job.setMapOutputClasses(Text.class, Text.class);
		job.run();
	}

	/**
	 * Use mapper to get similarity for individual dimension
     * Use reducer to sum all dimensions
	 * @param input
	 * @param output
	 * @throws IOException
	 * @throws ClassNotFoundException
	 * @throws InterruptedException
	 */
	private static void getHashtagAllSimilarities(String input, String output) 
        throws IOException,
			ClassNotFoundException, InterruptedException {
		
		Optimizedjob job = new Optimizedjob(new Configuration(), input, output,
				"Get similarities between all hashtags");
		job.setClasses(SimilarityAllMapper.class, SimilarityAllReducer.class, null);
		job.setMapOutputClasses(Text.class, IntWritable.class);
		job.run();
    }

    /**
     * Sort all hashtag pairs by similarities
     */
	private static void sortHashTagBySimilarity(String input, String output)
			throws Exception {
		Optimizedjob job = new Optimizedjob(new Configuration(), input, output,
				"Sort HashTage Pairs by Similarity");

		job.setClasses(HashtagSortMapper.class, null, null);
		job.setMapOutputClasses(IntWritable.class, Text.class);
		job.run();
	}
